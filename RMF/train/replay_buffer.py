...
class multiAgentReplayBuffer:  # for off-policy
    def __init__(self,
                 max_size: int,
                 state_dim: int,
                 action_dim: int,
        ...
        """
        self.states = th.empty((max_size, n_agents, state_dim), dtype=th.float32, device=self.device)
        self.actions = th.empty((max_size, n_agents, action_dim), dtype=th.float32, device=self.device)
        self.rewards = th.empty((max_size, n_agents), dtype=th.float32, device=self.device)
        self.dones = th.empty((max_size), dtype=th.float32, device=self.device)
    ...
    def sample(self, batch_size: int) -> Tuple[TEN, TEN, TEN, TEN, TEN, TEN]:
        ...
        ids = th.randint(sample_len, size=(batch_size,), requires_grad=False, device=self.device)
