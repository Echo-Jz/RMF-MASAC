...
from .run_init import TrainerInit
...
'''train'''
class TrainerBase(TrainerInit):
    def __init__(self,AgentClass,EnvClass,arglist):
        super().__init__(AgentClass,EnvClass,arglist)
        train_multiagent_single_process(args=self.args)
...
def train_multiagent_single_process(args: Config):
    ...
    if args.num_envs == 1:
        assert state.shape == (args.arglist.numAgent,args.state_dim)
    ...
    if args.if_off_policy:
        buffer = multiAgentReplayBuffer(
            gpu_id=args.gpu_id,
            n_agents=args.arglist.numAgent,
    ...
    if_train = True
    with open(cwd+'/args.json', 'w') as f:
        json.dump(vars(args.arglist), f, indent=4)
    del args
    while if_train:
        buffer_items = agent.explore_env(env, horizon_len)
        exp_r = buffer_items[2].mean(dim=0).squeeze()
        ...
        # progress = min(1.0, evaluator.total_step / break_step)
        # k1 = 0.018 * (1 - 0.6 * progress ** 0.7)  # 初始探索强度增加 ≈40%(15)
        # if progress < 0.4:  # 延长低约束阶段
        #     k2 = 0.0012
        # elif progress < 0.75:  # 平滑过渡阶段
        #     k2 = 0.0022
        # else:
        #     k2 = 0.0045  # 最终约束强度增加 ≈50%
        th.set_grad_enabled(True)
        logging_tuple = agent.update_net(buffer, k1, k2)
        th.set_grad_enabled(False)

        evaluator.multiagent_evaluate_and_save(agent=agent, steps=horizon_len, exp_r=exp_r, logging_tuple=logging_tuple)
