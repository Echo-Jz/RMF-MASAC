...
class Config:
    ...
    def init_before_training(self):
        ...
        if self.cwd is None:  # set cwd (current working directory) for saving model
            self.cwd = f'./new_{self.env_name}_RMF{self.agent_class.__name__[5:]}_random_seed_{self.random_seed}_explore_noise_std_{self.explore_noise_std}_num_agents_30_mixture'
...
def build_env(env_class=None, env_args: dict = None, gpu_id: int = -1):
    ...
    env = env_class(env_args['arglist'])
